...
void draw() {
  ...
  if (pantallaActual == 1) { //si el valor de pantallaActual es 1... 
    PrimeraPantalla(); //...se llama a la funcion que dibuja la primera pantalla D:
...
void mouseClicked() {
  if (pantallaActual == 1) { //primer boton
  ...
  } else if (pantallaActual == 7) { //boton de reinicio
    if (dist(mouseX, mouseY, width / 2, height / 2) < botonAncho / 2) { //verifica si se hizo click en la zona del boton
